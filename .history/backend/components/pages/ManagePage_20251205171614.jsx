...
const ManagePage = (props) => {
  ...
  const load = async (targetSlug) => {
    setLoading(true);
    try {
      const resp = await api.resourceAction({ resourceId, actionName: 'manage', params: { slug: targetSlug } });
  ...
  const pageOptions = useMemo(() => (pages || []).map(p => ({ value: p.slug, label: p.name })), [pages]);
  const selectedPageOption = useMemo(() => pageOptions.find(o => o.value === slug) || null, [pageOptions, slug]);
