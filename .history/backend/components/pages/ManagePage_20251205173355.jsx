...
const AdminPanel = () => {
  ...
  const [pageData, setPageData] = useState({ title: '', eyebrow: '', subtitle: '', heroImage: '', sections: [] });
  ...
  const load = async (targetSlug) => {
    setLoading(true);
    try {
      const resp = await api.resourceAction({ resourceId: 'Pages', actionName: 'manage', params: { slug: targetSlug } });
      const data = resp?.data?.data || {};
      setPages((data.pages || []).map(p => ({ value: p.slug, label: p.name })));
      if (data.page) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="w-64">
                <select
                  ...
                  value={slug}
                  onChange={(e)=>{ const v = e.target.value; setSlug(v); load(v); }}
                >
                  {pages.map(opt => (
        ...
        <main className="flex-1 overflow-auto p-8">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
              ...
              {/* Render existing sections content */}
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {(pageData.sections || []).slice(0,5).map((s, idx) => (
                  <div key={idx} className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6 border border-gray-200">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-xs font-mono text-gray-500">{s.key || `section-${idx+1}`}</span>
                      <span className="text-xs text-gray-400">Order: {s.order ?? idx}</span>
                    </div>
                    {s.type === 'heading' ? (
                      <h4 className="text-lg font-bold text-gray-800">{s.text || 'Heading'}</h4>
                    ) : s.type === 'paragraph' ? (
                      <p className="text-sm text-gray-700 whitespace-pre-wrap">{s.text || 'Body text...'}</p>
                    ) : s.type === 'list' ? (
                      ...
                      <div className="space-y-2">
                        {s.imageUrl ? (
                          <img src={s.imageUrl} alt={s.altText || ''} className="rounded-lg border border-gray-200" />
                        ) : (
                          <div className="h-32 bg-gray-100 rounded-lg border border-dashed border-gray-300 flex items-center justify-center text-gray-400">No Image</div>
                        )}
                        {s.altText && <div className="text-xs text-gray-500">Alt: {s.altText}</div>}
                      </div>
                    ) : s.type === 'cta' ? (
                      <a href={s.href || '#'} className="inline-flex items-center px-4 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700">{s.label || 'CTA'}</a>
                    ) : (
                      <p className="text-sm text-gray-700 whitespace-pre-wrap">{s.text || 'Content'}</p>
                    )}
                  </div>
                ))}
              </div>
