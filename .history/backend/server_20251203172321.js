...
let adminRouter;
if (process.env.ADMIN_PUBLIC === 'true' || process.env.DISABLE_ADMIN_AUTH === 'true') {
  // Public admin (no login) — only for local/dev usage
  adminRouter = AdminJSExpress.buildRouter(adminJs);
  console.warn('⚠️  AdminJS authentication DISABLED (public access enabled). Do NOT use in production.');
} else {
