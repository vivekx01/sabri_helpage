...
const ReadMorePage = ({ onBack }) => (
 ...
 const ElderlyIcon = (props) => <Users className="w-10 h-10 mx-auto text-green-600 mb-2" {...props} />;
const EducationIcon = (props) => <MessageCircle className="w-10 h-10 mx-auto text-blue-600 mb-2" {...props} />; // Using MessageCircle as placeholder for Education/Learning
const HealthIcon = (props) => <Heart className="w-10 h-10 mx-auto text-red-600 mb-2" {...props} />;
const VerifiedIcon = (props) => <Star className="w-4 h-4 mr-1 text-green-600" fill="currentColor" {...props} />;
const CheckMarkIcon = (props) => (
    // Defining a simple SVG checkmark since a Lucide 'Check' wasn't imported.
    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" {...props}>
...
// ... (All other existing pages: GalleryPage, MentalHealthPage, ElderlyCarePage, GirlEducationPage, ReadMorePage)
...
const DonatePage = ({ onBack, initialData }) => {
    ...
    const handleSubmit = (e) => {
        ...
        // Basic validation check (PAN is required in the form, and finalAmount > 0)
        if (!pan || finalAmount <= 0) {
             alert("Please ensure a valid donation amount and PAN number are provided.");
             return;
        }
    ...
    // ----------------------------------------------------------------------
    // --- RENDER STEP 2: Full-screen Donor Details Form (Completed) ---
    // ----------------------------------------------------------------------
    if (step === 2) {
        return (
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input id="province" type="text" value={province} onChange={(e) => setProvince(e.target.value)} className="w-full p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2" style={{ '--tw-ring-color': PRIMARY_DARK }} />
                                    </div>
                                    <div className='flex items-center pt-4'>
                                         <input id="existing-donor" type="checkbox" checked={isExistingDonor} onChange={(e) => setIsExistingDonor(e.target.checked)} className="h-4 w-4 rounded border-gray-300" style={{ accentColor: PRIMARY_DARK }} />
    ...
    // ----------------------------------------------------------------------
    // --- RENDER STEP 1: Donation Amount Selection (Original Step 1) ---
    // ----------------------------------------------------------------------
    return (
        <section className="py-16 md:py-24" style={{ backgroundColor: BG_LIGHT_GRAY }}>
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="bg-white p-6 sm:p-10 rounded-2xl shadow-2xl border-t-8" style={{borderColor: ACCENT_ORANGE}}>
                    <h1 className="text-4xl font-extrabold text-gray-900 mb-2">Support Our Cause</h1>
                    <p className="text-gray-600 mb-8">Choose your preferred amount and area of support.</p>

                    <h2 className="text-xl font-bold text-gray-800 mb-4">1. Select Donation Amount (INR)</h2>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
                        {donationOptions.map((opt) => (
                            <button
                                key={opt}
                                onClick={() => handleSelectAmount(opt)}
                                className={`p-4 rounded-lg font-bold text-lg transition duration-200 shadow-md ${
                                    amount === opt && !customAmount
                                        ? 'text-white border-2'
                                        : 'bg-gray-100 text-gray-800 hover:bg-gray-200 border-2 border-gray-300'
                                }`}
                                style={amount === opt && !customAmount ? { backgroundColor: ACCENT_ORANGE, borderColor: ACCENT_ORANGE } : {}}
                            >
                    ...
                    <div className="mb-8">
                        <label htmlFor="custom-amount" className="block text-sm font-medium text-gray-700 mb-2">Or Enter Custom Amount (Minimum ₹500)</label>
                        <div className="flex rounded-lg shadow-sm">
                            <span className="inline-flex items-center px-4 rounded-l-lg border border-r-0 border-gray-300 bg-gray-50 text-gray-500">₹</span>
                            <input
                                id="custom-amount"
                                type="number"
                                min="500"
                                ...
                                className={`flex-1 p-3 border-2 rounded-r-lg focus:outline-none focus:ring-2 ${isCustomActive ? 'border-orange-500' : 'border-gray-300'}`}
                    ...
                    <div className="flex items-center justify-between p-4 rounded-lg mb-8 shadow-inner" style={{backgroundColor: BG_LIGHT_GRAY}}>
                        <p className="font-semibold text-gray-700">Make this a Monthly Recurring Donation?</p>
                        <div className="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
                            ...
                            />
                             <label 
                                htmlFor="toggle" 
                                className={`toggle-label block overflow-hidden h-6 rounded-full cursor-pointer ${isRecurring ? 'bg-orange-400' : 'bg-gray-300'}`}
                            >
                                <span className={`absolute block w-4 h-4 mt-1 ml-1 rounded-full bg-white shadow inset-y-0 left-0 transition-transform duration-300 ease-in-out ${isRecurring ? 'translate-x-4' : 'translate-x-0'}`}></span>
                            </label>
                        </div>
                    </div>


                    <h2 className="text-xl font-bold text-gray-800 mb-4">2. Choose Area of Support</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
                        {/* Cause Selection Cards */}
                        {
                            [
                                { id: 'Elderly Care', icon: ElderlyIcon, color: 'green-600' },
                                { id: 'Girl Education', icon: EducationIcon, color: 'blue-600' },
                                { id: 'Mental Health', icon: HealthIcon, color: 'red-600' }
                            ].map((cause) => {
                                const IconComponent = cause.icon;
                                const isSelected = supportCategory === cause.id;

                                return (
                                    <div
                                        key={cause.id}
                                        onClick={() => setSupportCategory(cause.id)}
                                        className={`p-6 rounded-xl cursor-pointer border-4 transition duration-300 hover:shadow-lg text-center ${
                                            isSelected
                                                ? 'bg-white shadow-xl'
                                                : 'bg-gray-50 border-gray-200 hover:border-gray-400'
                                        }`}
                                        style={isSelected ? { borderColor: ACCENT_ORANGE } : {}}
                                    >
                                        <IconComponent />
                                        <h3 className={`font-semibold text-lg mt-2 ${isSelected ? 'text-gray-900' : 'text-gray-700'}`}>
                                            {cause.id}
                                        </h3>
                                    </div>
                                );
                    ...
                    <button
                        onClick={handleNext}
                        className={`w-full py-4 rounded-full text-xl font-bold text-white shadow-xl transition duration-300 hover:bg-opacity-90 flex items-center justify-center ${finalAmount > 0 ? '' : 'opacity-50 cursor-not-allowed'}`}
                        style={{ backgroundColor: PRIMARY_DARK }}
                        disabled={finalAmount === 0 || finalAmount < 500}
                    >
                        Proceed to Donor Details <ArrowRight className="w-5 h-5 ml-2" />
                    </button>
                    <p className="text-sm text-center text-red-500 mt-3">
                         {finalAmount < 500 && finalAmount > 0 ? "Minimum donation is ₹500" : (finalAmount === 0 ? "Please select a donation amount to proceed." : <VerifiedIcon />)}
                         {finalAmount >= 500 && "Amount selected. Ready to proceed."}
                    </p>
                </div>
            </div>
        </section>
