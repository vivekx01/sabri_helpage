...
const ReadMorePage = ({ onBack }) => (
 ...
 const DonatePage = ({ onBack, initialData }) => {
    ...
    // --- Step 2 State ---
    // Note: The image shows a single form. I'll make the first view Step 1/HomeForm, and the second view Step 2/DetailsForm.
    ...
    const donationOptions = [5000, 10000, 25000]; // Used in Step 2 preview
    const finalAmount = customAmount ? parseFloat(customAmount) : amount || 0; 
    
    // --- Step 1 -> Step 2 Navigation (from HomeDonationForm) ---
    const handleNextStep = (data) => {
        setAmount(data.amount);
        setCustomAmount(data.customAmount);
        setSupportCategory(data.category);
        setStep(2);
    };
    ...
    const handleSubmit = (e) => {
        ...
        console.log(`Thank you, ${firstName}! Your donation of ₹${finalAmount.toFixed(2)} (${isRecurring ? 'Monthly' : 'One-time'}) for ${supportCategory} has been processed.`);
        
        // Simulating completion by going back to the home view or showing a success message
        // For this example, we'll reset to step 1 (HomeForm) after a small delay
        setTimeout(() => {
            setStep(1);
            alert('Donation complete! Thank you!');
            // onBack(); // If you want to go back to the application's home page
        }, 500);
    };

    // The entire page content is wrapped here
    return (
        <div className="min-h-screen">
            {/* -------------------------------------------
              1. HERO SECTION (Orange Gradient)
              -------------------------------------------
            */}
            <header className="py-16 sm:py-24 text-center text-white" style={{ background: BG_ORANGE_GRADIENT }}>
            ...
            */}
            <section className="py-16 bg-white">
                <div className="max-w-6xl mx-auto px-4 text-center">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {/* Elderly Care */}
                        <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition duration-300">
                            <ElderlyCareIcon style={{ color: PRIMARY_ORANGE }} />
                            <h3 className="text-xl font-semibold text-gray-800 mt-4 mb-2">Elderly Care</h3>
                            <p className="text-sm text-gray-500">Dignity and support for senior citizens.</p>
                        </div>
                        {/* Education */}
                        <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition duration-300">
                            <EducationIcon style={{ color: PRIMARY_ORANGE }} />
                            <h3 className="text-xl font-semibold text-gray-800 mt-4 mb-2">Education</h3>
                            <p className="text-sm text-gray-500">Education and girl child.</p>
                        </div>
                        {/* Health */}
                        <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition duration-300">
                            <HealthIcon style={{ color: PRIMARY_ORANGE }} />
                            <h3 className="text-xl font-semibold text-gray-800 mt-4 mb-2">Health</h3>
                            <p className="text-sm text-gray-500">Ensuring mental health and healthcare.</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            {/* -------------------------------------------
              3. IMPACT AND DONATION FORM SECTION
              -------------------------------------------
            */}
            <section className="py-20" style={{ backgroundColor: BG_LIGHT_GRAY }}>
                <div className="max-w-6xl mx-auto px-4">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                        
                        {/* Left Side: Your Support Creates Impact */}
                        <div className="lg:pr-10">
                            <h2 className="text-4xl font-extrabold text-gray-800 mb-6">
                                Your Support Creates <span style={{ color: PRIMARY_ORANGE }}>Impact</span>
                            </h2>
                            ...
                            {/* Transparency Box */}
                            <div className="p-6 rounded-lg border-2 border-green-500 bg-white inline-block">
                                <p className="text-lg font-bold text-green-700">
                                    <CheckMarkIcon style={{color: 'rgb(34 197 94)'}} className="inline w-5 h-5 mr-1" />
                        ...
                        {/* Right Side: Donation Form (Conditional Rendering based on step) */}
                        <div className="lg:pl-10">
                            {/* Renders the simplified form from the image for Step 1 */}
                            {step === 1 && <HomeDonationForm onNextStep={handleNextStep} />}

                            {/* Renders the detailed form for Step 2 (your existing complex code) */}
                            {step === 2 && (
                                <div className="bg-white p-6 sm:p-10 rounded-2xl shadow-2xl border-t-8" style={{borderColor: PRIMARY_ORANGE}}>
                                    
                                    <h2 className="text-3xl font-extrabold text-gray-900 mb-2">Complete Your Donation</h2>
                                    ...
                                    <form onSubmit={handleSubmit} className="space-y-8">
                                        ...
                                        <div className="p-4 rounded-lg bg-yellow-50 border-l-4" style={{borderColor: PRIMARY_ORANGE}}>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-4">
                                                <button
                                                    ...
                                                    style={citizenType === 'indian' ? { backgroundColor: PRIMARY_ORANGE } : {}}
                                                >
                                                ...
                                                <button
                                                    ...
                                                    style={citizenType === 'foreign' ? { backgroundColor: PRIMARY_ORANGE } : {}}
                                                >
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input id="first-name" type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} required className="w-full p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2" style={{ '--tw-ring-color': PRIMARY_ORANGE }} />
                                            </div>
                                            <div>
                                                <label htmlFor="last-name" className="block text-sm font-medium text-gray-700 mb-1">Last Name *</label>
                                                <input id="last-name" type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} required className="w-full p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2" style={{ '--tw-ring-color': PRIMARY_ORANGE }} />
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2" style={{ '--tw-ring-color': PRIMARY_ORANGE }} />
                                            </div>
                                            <div>
                                                <label htmlFor="mobile" className="block text-sm font-medium text-gray-700 mb-1">Mobile Number *</label>
                                                <input id="mobile" type="tel" value={mobile} onChange={(e) => setMobile(e.target.value)} required className="w-full p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2" style={{ '--tw-ring-color': PRIMARY_ORANGE }} />
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input id="dob" type="date" placeholder="dd-mm-yyyy" value={dob} onChange={(e) => setDob(e.target.value)} className="w-full p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2" style={{ '--tw-ring-color': PRIMARY_ORANGE }} />
                                            </div>
                                            <div>
                                                <label htmlFor="pan" className="block text-sm font-medium text-gray-700 mb-1">PAN Number *</label>
                                                <input id="pan" type="text" value={pan} onChange={(e) => setPan(e.target.value)} required className="w-full p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2" style={{ '--tw-ring-color': PRIMARY_ORANGE }} />
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <input id="address" type="text" value={address} onChange={(e) => setAddress(e.target.value)} required className="w-full p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2" style={{ '--tw-ring-color': PRIMARY_ORANGE }} />
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input id="zip" type="text" value={zip} onChange={(e) => setZip(e.target.value)} required className="w-full p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2" style={{ '--tw-ring-color': PRIMARY_ORANGE }} />
                                            </div>
                                            <div>
                                                <label htmlFor="city" className="block text-sm font-medium text-gray-700 mb-1">City *</label>
                                                <input id="city" type="text" value={city} onChange={(e) => setCity(e.target.value)} required className="w-full p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2" style={{ '--tw-ring-color': PRIMARY_ORANGE }} />
                                            </div>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <input id="province" type="text" value={province} onChange={(e) => setProvince(e.target.value)} required className="w-full p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2" style={{ '--tw-ring-color': PRIMARY_ORANGE }} />
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-6">
                                                <div className="flex items-center">
                                                    <input id="existing-yes" name="existing-donor" type="radio" checked={isExistingDonor === true} onChange={() => setIsExistingDonor(true)} className="h-4 w-4" style={{ accentColor: PRIMARY_ORANGE }} />
                                                    <label htmlFor="existing-yes" className="ml-2 block text-sm text-gray-900">Yes</label>
                                                </div>
                                                <div className="flex items-center">
                                                    <input id="existing-no" name="existing-donor" type="radio" checked={isExistingDonor === false} onChange={() => setIsExistingDonor(false)} className="h-4 w-4" style={{ accentColor: PRIMARY_ORANGE }} />
