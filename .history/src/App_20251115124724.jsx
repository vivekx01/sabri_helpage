...
const ReadMorePage = ({ onBack }) => (
 ...
 const DonatePage = ({ onBack, initialData }) => {
    ...
    const [amount, setAmount] = useState(initialData?.amount || 5000); // Changed default to match new options
    ...
    // --- Step 2 State ---
    const [step, setStep] = useState(initialData ? 2 : 1); // Start at step 2 if initialData is provided
    const [citizenType, setCitizenType] = useState('indian');
    const [firstName, setFirstName] = useState('');
    const [lastName, setLastName] = useState('');
    const [email, setEmail] = useState(''); // Kept from original
    ...
    const donationOptions = [5000, 10000, 25000]; // Updated to match new design's defaults
    ...
    // Corrected logic: Use customAmount if present, otherwise use amount. Fallback to 0.
    ...
    const handleSubmit = (e) => {
        ...
        const allDonationData = {
            amount: finalAmount,
            isRecurring,
            supportCategory, // Added support category
            citizenType,
            firstName,
            lastName,
            email,
        ...
        // IMPORTANT: Replaced alert() with a console log.
        // alert() is bad practice in modern apps and was blocking.
    ...
    return (
        <section className="py-20 md:py-28" style={{ backgroundColor: BG_LIGHT_GRAY }}>
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                <button onClick={onBack} className={`text-sm font-semibold mb-8 flex items-center text-gray-700 hover:text-gray-900 transition`}>
                    <ChevronRight className="w-5 h-5 rotate-180 mr-1" /> Back to Home
                </button>
                {/* Changed border color to PRIMARY_DARK */}
                <div className="bg-white p-6 sm:p-10 rounded-2xl shadow-2xl border-t-8" style={{borderColor: PRIMARY_DARK}}>
                    
                    {/* Header is constant for both steps */}
                    <h1 className="text-4xl font-extrabold text-gray-900 mb-2">Make a Donation</h1>
                    <p className="text-gray-600 mb-10">
                        {step === 1 ? "Your contribution fuels our mission. Thank you for your kindness." : "Please provide your details to complete the donation."}
                    </p>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8">
                        ...
                        {step === 1 && (
                            <>
                                {/* 1. Donation Amount */}
                                <div>
                                    <label className="block text-xl font-bold text-gray-700 mb-3">Choose Amount</label>
                                    <div className="flex flex-wrap justify-center sm:justify-start gap-3 sm:gap-4 mb-4">
                                        {donationOptions.map((opt) => (
                                            <button
                                                ...
                                                className={`px-4 sm:px-6 py-2 sm:py-3 rounded-full text-base sm:text-lg font-bold transition duration-200 flex-grow sm:flex-grow-0 ${
                                                    amount === opt && !customAmount
                                                        ? `text-white shadow-lg`
                                                        : `text-gray-800 border-2 border-gray-300 hover:border-gray-500`
                                                }`}
                                                // Kept orange for selected amount as it's a clear indicator
                                    ...
                                    <input
                                        ...
                                        className="w-full mt-4 p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2"
                                        style={{ '--tw-ring-color': PRIMARY_DARK }} // Changed focus ring to dark
                                    />
                                    {finalAmount <= 0 && <p className="text-red-500 mt-2">Please select or enter a donation amount.</p>}
                                </div>

                                {/* 2. Frequency */}
                                <div>
                                    <label className="block text-xl font-bold text-gray-700 mb-3">Frequency</label>
                                    <div className="flex gap-4">
                                        <button
                                            type="button"
                                            onClick={() => setIsRecurring(false)}
                                            className={`flex-1 p-4 rounded-lg font-semibold transition duration-200 ${
                                                !isRecurring ? `text-white shadow-lg` : `bg-gray-100 text-gray-800 border-2 border-gray-300 hover:border-gray-500`
                                            }`}
                                            style={!isRecurring ? { backgroundColor: PRIMARY_DARK } : {}}
                                        >
                                            One-time
                                        </button>
                                        <button
                                ...
                                {/* 3. Support Category (Moved to Step 1 for better flow, matching HomeDonationForm) */}
                                <div>
                                    <label htmlFor="support-category" className="block text-xl font-bold text-gray-700 mb-3">I want to support</label>
                                    <select
                                        id="support-category"
                                        value={supportCategory}
                                        onChange={(e) => setSupportCategory(e.target.value)}
                                        className="w-full p-3 border-2 border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 text-gray-800"
                                ...
                                <div className="pt-4">
                                    <button
                                        ...
                                        className={`w-full py-4 rounded-full text-xl font-bold text-white transition duration-300 shadow-xl disabled:bg-gray-400`}
                                        style={{ backgroundColor: PRIMARY_DARK }} // Changed button color
                                    >
                                        Next
                                    </button>
                                </div>
                        ...
                        {/* --- STEP 2: Donor Details (from image) --- */}
                        {step === 2 && (
                            <div className="space-y-6">
                                ...
                                <div className="flex flex-col sm:flex-row gap-4 pt-4">
                                    ...
                                    <button
                                        ...
                                        style={{ backgroundColor: PRIMARY_DARK }} // Using dark color for "Pay Now" as in image
                                    >
...
const HomeDonationForm = ({ onDonate }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-xl border-t-8" style={{borderColor: PRIMARY_DARK}}>
            <h3 className="text-xl font-bold text-gray-900 mb-4">Support Our Cause</h3>
            <div className="flex flex-wrap gap-2 mb-4">
                {donationOptions.map((opt) => (
                    <button
                        ...
                        type="button"
                        onClick={() => handleSelectAmount(opt)}
                        className={`flex-1 px-4 py-2 rounded-lg text-sm font-bold transition duration-200 ${
                            amount === opt && !customAmount
            ...
            <input
                ...
                value={customAmount}
                onChange={handleCustomAmountChange}
                className="w-full mb-4 p-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:ring-2"
                style={{ '--tw-ring-color': PRIMARY_DARK }}
            />
            ...
            <p className="text-xs text-gray-500 my-3">Minimum amount should be Rs. 100/-</p>
