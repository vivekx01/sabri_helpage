...
const ReadMorePage = ({ onBack }) => (
 ...
 const ElderlyIcon = (props) => <Users className="w-16 h-16 mx-auto mb-3" style={{ color: '#10B981' }} {...props} />; // Example: Emerald-600
const EducationIcon = (props) => <Lightbulb className="w-16 h-16 mx-auto mb-3" style={{ color: '#3B82F6' }} {...props} />; // Example: Blue-500
const HealthIcon = (props) => <Heart className="w-16 h-16 mx-auto mb-3" style={{ color: '#EF4444' }} {...props} />; // Example: Red-500
const CheckMarkIcon = (props) => <CheckCircle className="w-5 h-5 mr-2 text-green-500" {...props} />; // For the verified text
const VerifiedIcon = (props) => <Star className="w-4 h-4 mr-1 text-green-600" fill="currentColor" {...props} />;

// --- The Complete DonatePage Component, matching the image layout ---
const DonatePage = ({ onBack, initialData }) => {
    // Donation form states (as per your previous code)
    ...
    const [isRecurring, setIsRecurring] = useState(false); // Not visible in image, but kept from previous code
    const [supportCategory, setSupportCategory] = useState(initialData?.category || 'Elderly Care'); // Default matches image default
    ...
    const handleCustomAmountChange = (e) => {
        ...
        setAmount(0); // Clear selected fixed amount when custom is used
    };
    ...
    const handleSubmitDonation = (e) => {
        ...
        // In a real app, this would lead to a payment gateway or detailed form
        alert(`Thank you for your donation of ₹${finalAmount.toLocaleString('en-IN')} towards ${supportCategory}!`);
        // Optionally, reset form or navigate away
        onBack(); // Go back to the previous page (e.g., Home)
    };

    return (
        <div className="min-h-screen bg-white">
            {/* --- Hero Section: Transform Lives, Empower Communities (Matches top of image) --- */}
            <section className="py-20 text-white text-center" style={{ backgroundColor: ACCENT_ORANGE }}>
            ...
            {/* --- Our Focus Areas Section (Matches middle of image) --- */}
            <section className="py-20 bg-white">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {/* Elderly Care */}
                        <div className="bg-white p-8 rounded-xl shadow-lg border-t-4" style={{ borderColor: '#10B981' }}> {/* Emerald-600 */}
                            <ElderlyIcon />
                        ...
                        {/* Education */}
                        <div className="bg-white p-8 rounded-xl shadow-lg border-t-4" style={{ borderColor: '#3B82F6' }}> {/* Blue-500 */}
                            <EducationIcon />
                            <h3 className="text-xl font-bold text-gray-900 mb-2">Education</h3>
                            <p className="text-gray-600">Education and girl child.</p>
                        </div>
                        {/* Health */}
                        <div className="bg-white p-8 rounded-xl shadow-lg border-t-4" style={{ borderColor: '#EF4444' }}> {/* Red-500 */}
                            <HealthIcon />
            ...
            {/* --- Your Support Creates Impact Section (Matches bottom of image) --- */}
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                    ...
                    <div>
                        ...
                        <div className="p-6 rounded-xl border border-gray-300 bg-white shadow-sm">
                            <p className="flex items-center text-md font-semibold text-gray-800 mb-2">
                                <CheckMarkIcon /> 100% Transparent | Verified NGO | Direct Impact
                            </p>
                    ...
                    {/* Right side: Donation Form (Matches image layout) */}
                    <div className="p-6 rounded-xl shadow-2xl" style={{ backgroundColor: BG_LIGHT_GRAY, borderTop: `8px solid ${ACCENT_ORANGE}` }}>
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-4">
                            {donationOptions.map((opt) => (
                                <button
                                    ...
                                    className={`py-3 rounded-lg font-semibold text-lg transition duration-200 ${
                                        amount === opt && !customAmount
                                            ? 'text-white'
                                            : 'bg-gray-100 text-gray-800 hover:bg-gray-200 border border-gray-300'
                                    }`}
                                    style={amount === opt && !customAmount ? { backgroundColor: ACCENT_ORANGE } : {}}
                                >
                        ...
                        <div className="mb-4">
                            <div className="flex rounded-lg border border-gray-300">
                                <span className="inline-flex items-center px-3 rounded-l-lg border-r border-gray-300 bg-gray-50 text-gray-500 text-sm">₹</span>
                                <input
                                    type="number"
                                    min="500"
                                    value={customAmount}
                                    onChange={handleCustomAmountChange}
                                    className={`flex-1 p-2 rounded-r-lg focus:outline-none focus:ring-1 ${isCustomActive ? 'border-orange-500 ring-orange-500' : ''}`}
                        ...
                        {/* I offer my support for dropdown */}
                        <div className="mb-6">
                            ...
                            <select
                                ...
                                className="w-full p-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-orange-500"
                            >
                                <option value="Elderly Care">Elderly Care</option>
                                <option value="Girl Education">Girl Education</option>
                        ...
                        <button
                            onClick={() => alert("Login functionality not implemented.")} // Placeholder
                            className="w-full py-3 rounded-lg font-bold text-white transition duration-200 bg-gray-700 hover:bg-gray-800 mb-6"
                        >
                            For Regular Users Login <ArrowRight className="inline-block ml-2 w-4 h-4" />
                        </button>

                        {/* Next Button */}
                        <button
                            onClick={handleSubmitDonation} // Direct submit for now, can be changed to go to Step 2
                            className={`w-full py-3 rounded-lg font-bold text-white transition duration-200 ${
                                finalAmount >= 500
                                    ? 'bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600' // Stylish gradient for 'Next'
...
// ----------------------------------------------------------------------
// --- 4. THE MAIN APP COMPONENT (To render the DonatePage) ---
// ----------------------------------------------------------------------

const App = () => {
    const [currentPage, setCurrentPage] = useState('Donate'); // Start directly on the DonatePage
    const [initialDonateData, setInitialDonateData] = useState(null);

    const navigate = (page, data = null) => {
        setInitialDonateData(data);
        setCurrentPage(page);
    };
    ...
    const renderPage = () => {
        switch (currentPage) {
            case 'Home':
                return <HomePage onDonateClick={(data) => navigate('Donate', data)} />;
            case 'Donate':
                return <DonatePage onBack={() => navigate('Home')} initialData={initialDonateData} />;
            case 'Gallery':
                return <GalleryPage onBack={() => navigate('Home')} />;
            case 'MentalHealth':
                return <MentalHealthPage onBack={() => navigate('Home')} />;
            case 'ElderlyCare':
                return <ElderlyCarePage onBack={() => navigate('Home')} />;
            default:
                return <HomePage onDonateClick={(data) => navigate('Donate', data)} />;
        }
    };
    ...
    return (
        <div className="font-sans antialiased">
            {/* You can add a Header here if you want navigation above the Donate page */}
            {/* <Header currentPage={currentPage} onNavigate={navigate} /> */}
            <main>
                {renderPage()}
            </main>
            {/* You can add a Footer here if you want it below the Donate page */}
            {/* <Footer /> */}
        </div>
    );
};
