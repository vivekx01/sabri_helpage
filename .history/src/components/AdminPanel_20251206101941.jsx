        ...
        header: { title: 'About', subtitle: 'Founding, purpose and governing body' },
                      <div className="space-y-6">
                        <div className="flex items-center justify-between">
                          <h4 className="text-sm font-semibold text-gray-900">Sections</h4>
                          <div className="flex gap-2">
                            <button className="px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-medium hover:bg-indigo-700" onClick={() => setSections([...sections, { type: 'richText', html: '' }])}>Add Rich Text</button>
                            <button className="px-3 py-1.5 bg-gray-800 text-white rounded-lg text-xs font-medium hover:bg-gray-900" onClick={() => setSections([...sections, { type: 'image', imageUrl: '', altText: '' }])}>Add Image</button>
                            <button className="px-3 py-1.5 bg-amber-600 text-white rounded-lg text-xs font-medium hover:bg-amber-700" onClick={() => setSections([...sections, { type: 'list', items: [] }])}>Add List</button>
                            <button className="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-xs font-medium hover:bg-emerald-700" onClick={() => setSections([...sections, { type: 'cta', label: '', href: '' }])}>Add CTA</button>
                          </div>
                        </div>
                              ...
                              <div key={idx} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <select className="px-2 py-1 bg-white border border-gray-200 rounded text-xs" value={sec.type} onChange={(e) => onChange({ type: e.target.value })}>
                                  ...
                                  <div className="flex items-center gap-2">
                                    <button className="px-2 py-1 text-xs text-gray-600 hover:bg-gray-50 rounded" onClick={() => setSections([...sections.slice(0, idx + 1), sections[idx], ...sections.slice(idx + 1)])}>Duplicate</button>
                                    <button className="px-2 py-1 text-xs text-red-600 hover:bg-red-50 rounded" onClick={() => setSections(sections.filter((_, i) => i !== idx))}>Remove</button>
                                  </div>
                                </div>
                                {sec.type === 'image' && (
                                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <input className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm" placeholder="Image URL" value={sec.imageUrl || ''} onChange={(e) => onChange({ imageUrl: e.target.value })} />
                                    <input className="px-3 py-2 bg_white border border-gray-200 rounded-lg text-sm" placeholder="Alt Text" value={sec.altText || ''} onChange={(e) => onChange({ altText: e.target.value })} />
                                  </div>
                                )}
                                {sec.type === 'cta' && (
                                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <input className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm" placeholder="Button Label" value={sec.label || ''} onChange={(e) => onChange({ label: e.target.value })} />
                                    <input className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm" placeholder="Button Link (/route)" value={sec.href || ''} onChange={(e) => onChange({ href: e.target.value })} />
                                  </div>
                                )}
                                {sec.type === 'list' && (
                                  <textarea className="mt-3 w-full min-h-24 bg-white border border-gray-200 rounded-lg text-sm px-3 py-2" placeholder="One item per line" value={(Array.isArray(sec.items) ? sec.items.join('\n') : sec.items || '')} onChange={(e) => onChange({ items: e.target.value.split('\n').filter(Boolean) })} />
                                )}
                                {(sec.type === 'richText' || sec.type === 'paragraph') && (
                                  <textarea className="mt-3 w-full min-h-28 bg-white border border-gray-200 rounded-lg text-sm px-3 py-2" placeholder={sec.type === 'paragraph' ? 'Paragraph text...' : 'HTML / Rich text...'} value={sec.type === 'paragraph' ? (sec.text || '') : (sec.html || '')} onChange={(e) => onChange(sec.type === 'paragraph' ? { text: e.target.value, html: undefined } : { html: e.target.value })} />
                                )}
                                <div className="mt-2 text-xs text-gray-400">Type: {sec.type}</div>
                              </div>
                            );
                          })}
                        </div>
