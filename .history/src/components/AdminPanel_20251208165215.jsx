...
import {
  ...
  BookOpen, Award, Calendar, HelpCircle, Camera, FileImage, Book, Code
} from 'lucide-react';
...
const AdminPanel = () => {
  ...
  if (!API_BASE && typeof window !== 'undefined') {
    // Use same-origin '/api' when not configured
    API_BASE = '/api';
  }
  ...
  const menuItems = [
    ...
    { id: 'frontend-code', label: 'Frontend Code', icon: Edit },
...
// Frontend code editor for simple file edits via backend endpoints
const FrontendCodeEditor = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await fetch(`${API_BASE}/admin/frontend-files`, { headers: getAuthHeaders() });
  ...
  const loadFile = async (fileId) => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE}/admin/frontend-files/${encodeURIComponent(fileId)}`, { headers: getAuthHeaders() });
  ...
  const saveFile = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${API_BASE}/admin/frontend-files/${encodeURIComponent(selected)}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            {files.map((f) => (
              <li key={f.id}>
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg border ${selected === f.id ? 'border-indigo-300 bg-indigo-50' : 'border-gray-200 hover:bg-gray-50'}`}
                >
                  <span className="text-sm font-mono">{f.name}</span>
                  <span className="block text-xs text-gray-500">{f.path}</span>
                </button>
              </li>
            ))}
          </ul>
      ...
      <div className="lg:col-span-3 bg-white rounded-xl p-4 border border-gray-200">
        {selected ? (
