...
const AdminPanel = () => {
  ...
  const handleOpenPage = (slug) => {
    ...
    setPageDoc(doc);
    setSections(Array.isArray(doc?.sections) ? doc.sections : [{ type: 'richText', html: '' }]);
  };
  ...
  const handleSave = () => {
    ...
    const payload = {
      ...
      sections: sections && sections.length ? sections : [{ type: 'richText', html: bodyTextarea?.value || '' }],
      status: 'published',
    };
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            ...
                            onClick={() => setSections([...sections, { type: 'richText', html: '' }])}
                          >
                            Add Section
                          </button>
                        </div>
                          ...
                          {sections.map((sec, idx) => (
                            <div key={idx} className="bg-white border border-gray-200 rounded-lg p-4">
                              ...
                              {sec.type === 'image' ? (
                                ...
                                <textarea
                                  className="mt-3 w-full min-h-24 bg-white border border-gray-200 rounded-lg text-sm px-3 py-2"
                                  placeholder="One item per line"
                                  value={(Array.isArray(sec.items) ? sec.items.join('\n') : sec.items || '')}
                                  onChange={(e) => {
                                    const next = [...sections];
                                ...
                                <textarea
                                  className="mt-3 w-full min-h-28 bg-white border border-gray-200 rounded-lg text-sm px-3 py-2"
                                  placeholder={sec.type === 'paragraph' ? 'Paragraph text...' : 'HTML / Rich text...'}
                                  value={sec.html || sec.text || ''}
                                  onChange={(e) => {
                                    const next = [...sections];
                                    if (sec.type === 'paragraph') next[idx] = { ...next[idx], text: e.target.value, html: undefined };
                                    else next[idx] = { ...next[idx], html: e.target.value };
                                    setSections(next);
                                  }}
                                />
                              )}
              ...
              <div className="max-w-4xl">
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden divide-y divide-gray-100">
                  <div className="p-6 sm:p-8">
                    ...
                    <div className="space-y-4">
                      {['Super Admin', 'Admin', 'Manager', 'Editor'].map((role, idx) => (
