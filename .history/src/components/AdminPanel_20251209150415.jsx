...
const AdminPanel = () => {
  ...
  // Authentication check - CRITICAL: This must run first
  useEffect(() => {
  ...
  // Fetch pages list from backend - only when authenticated
  useEffect(() => {
    if (activeSection !== 'pages' || !isAuthenticated) return;

    const fetchPages = async () => {
      setLoading(true);
      setError('');
      try {
        const pages = await api.getPagesList();
        setPagesList(Array.isArray(pages) ? pages : []);
      } catch (error) {
        console.error('Failed to fetch pages:', error);
        setError(error.message || 'Failed to load pages. Please try again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchPageContent = async () => {
      ...
      setError('');
      try {
        const content = await api.getPageContent(selectedPage.toLowerCase());
        setPageContent(content || {});
      } catch (error) {
        console.error(`Failed to fetch content for ${selectedPage}:`, error);
        setError(error.message || 'Failed to load page content. Please try again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const pages = pagesList.length > 0 
    ? pagesList.map(p => p.slug || p) 
    : Object.keys(pageTemplates);
    
  const filteredPages = pages.filter(page => 
    page && typeof page === 'string' && page.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleSaveSection = async (sectionId, data) => {
    ...
    try {
      ...
      await api.updatePageContent(selectedPage.toLowerCase(), updatedContent);
      setPageContent(updatedContent);
      
      return { success: true };
    } catch (error) {
      console.error('Failed to save section:', error);
      setError(error.message || 'Failed to save section. Please try again.');
  ...
  const handlePageAction = async (action, page, section) => {
    try {
      setLoading(true);
      setError('');
      ...
      switch (action) {
        case 'save':
          ...
          await api.updatePageContent(page.toLowerCase(), pageContent);
          alert('Page updated successfully!');
          break;
          
        case 'publish':
          await api.updatePageContent(page.toLowerCase(), {
            ...pageContent,
            status: 'published',
        ...
        case 'preview':
          ...
          window.open(`/preview/${page.toLowerCase()}`, '_blank');
          break;
      ...
      setError(error.message || `Failed to ${action} page. Please try again.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Don't render anything if not authenticated (redirect happens above)
  if (!isAuthenticated) {
    return null;
  }
