...
const Footer = () => {
    ...
    const quickLinks = [
        [
            ...
            { label: "Clubs (SSIC)", href: "/ilc" },
            { label: "Awards", href: "/awards" }
        ],
        [
            { label: "Support a Cause", href: "/donate" },
            { label: "Elderly Care", href: "/elderly-care" },
            { label: "Mental Health", href: "/mental-health" },
            { label: "Girl Child", href: "/girl-education" }
        ],
        [
                        ...
                        {quickLinks.map((column, colIndex) => (
                            <ul key={colIndex} className={ulClass}>
                                {column.map((link, linkIndex) => (
                                    <li key={linkIndex}>
                                        <a href={link.href} className={linkClass} onClick={e => {
                                            if (link.href === '/donate' && typeof window !== 'undefined' && window.onNavigate) {
                                                e.preventDefault();
                                                window.onNavigate('donate');
                                            }
                                        }}>
                                            {link.label}
                                        </a>
            ...
            <div className="mt-12 pt-6 border-t border-gray-800 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
                    ...
                    <div className="order-2 md:order-1">
                        <a href="/gallery" className="text-gray-400 hover:text-orange-400 transition duration-300 font-semibold text-lg">Gallery</a>
                    </div>
                    ...
                    <div className="order-3 w-full md:w-auto flex md:justify-end justify-end">
                        <div className="flex flex-row items-center space-x-6">
                            <a href="/about" className="text-gray-300 hover:text-orange-400 text-sm transition duration-300">About</a>
                            <a href="/terms" className="text-gray-300 hover:text-orange-400 text-sm transition duration-300">Terms</a>
                            <a href="/privacy" className="text-gray-300 hover:text-orange-400 text-sm transition duration-300">Privacy</a>
                        </div>
                    </div>
                </div>
            </div>
