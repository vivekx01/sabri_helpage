...
const AdminPanel = () => {
  ...
  // Backend wiring
  let API_BASE = '';
  try { API_BASE = (import.meta?.env?.VITE_API_BASE_URL || '').replace(/\/$/, ''); } catch (e) { API_BASE = ''; }
  if (!API_BASE && typeof window !== 'undefined') { API_BASE = '/api'; }
  const authHeaders = () => ({ 'Content-Type': 'application/json', 'Authorization': `Bearer ${localStorage.getItem('adminToken')}` });

  // Pages data from backend
  const [pagesList, setPagesList] = useState([]);
  const [pageDoc, setPageDoc] = useState(null);
  useEffect(() => {
    if (activeSection === 'pages') {
      fetch(`${API_BASE}/admin/pages`, { headers: authHeaders() })
        .then(r => r.json()).then(setPagesList).catch(() => setPagesList([]));
    }
  }, [activeSection]);
  useEffect(() => {
    if (activeSection === 'pages' && selectedPage) {
      fetch(`${API_BASE}/admin/pages/${encodeURIComponent(selectedPage.toLowerCase())}`, { headers: authHeaders() })
        .then(r => r.json()).then(setPageDoc).catch(() => setPageDoc(null));
    }
    ...
    <div className="flex h-screen bg-gradient-to-br from-gray-50 via-white to-indigo-50">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {activeSection === 'pages' && !selectedPage && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {pagesList
                    .map(p => p.slug)
                    .filter(slug => slug.toLowerCase().includes(searchTerm.toLowerCase()))
                    .map((slug, idx) => (
                      <button
                        key={idx}
            ...
            {activeSection === 'pages' && selectedPage && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <button
                    onClick={() => { setSelectedPage(null); setEditingSection(null); }}
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                  ...
                  <p className="text-sm text-gray-600">Edit content sections only (not full page code)</p>
                </div>

                <div className="space-y-4">
                  {(pageDoc?.sections || []).map((section, i) => (
                    <div key={i} className="bg-white rounded-xl border border-gray-200 p-6">
                      <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                          <div className="p-2 bg-indigo-50 rounded-lg"><GripVertical className="text-indigo-600" size={18} /></div>
                          <div>
                            <h4 className="font-semibold text-gray-900">{section.type || 'Section'}</h4>
                            <p className="text-xs text-gray-500">Editable content fields</p>
                          </div>
                        </div>
                        <button className="px-3 py-1.5 text-sm bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg">Edit</button>
                      </div>

                      <div className="space-y-3">
                        <label className="block text-sm font-medium text-gray-700">Heading</label>
                        <input type="text" defaultValue={section.heading || ''} className="w-full px-4 py-3 border rounded-lg" />
                        <label className="block text-sm font-medium text-gray-700">Subheading</label>
                        <input type="text" defaultValue={section.subheading || ''} className="w-full px-4 py-3 border rounded-lg" />
                        <label className="block text-sm font-medium text-gray-700">Content</label>
                        <textarea rows="6" defaultValue={section.content || ''} className="w-full px-4 py-3 border rounded-lg" />
                      </div>
                    </div>
                  ))}
                </div>
        ...
        {selected ? (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-gray-900">Editing: {selected}</h3>
              <button onClick={saveFile} disabled={saving} className="px-4 py-2 bg-indigo-600 text-white rounded-lg disabled:opacity-50">
                {saving ? 'Saving...' : 'Save'}
              </button>
            </div>
            <textarea
              ...
              className="w-full h-[60vh] font-mono text-sm border border-gray-300 rounded-lg p-3"
            />
          </div>
        ) : (
