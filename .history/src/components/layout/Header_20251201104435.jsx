      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-1 md:py-2">
          ...
          <div className="hidden lg:flex items-center space-x-8">
            <nav className="flex space-x-8 text-sm font-semibold uppercase tracking-normal text-gray-700">
              {navItems.map(item => (
                <a
                  key={item.name}
                  href="#"
                  onClick={(e) => { e.preventDefault(); handleNavClick(item.page); }}
                  className="hover:text-[#FF8F5C] transition-colors duration-200 relative group py-1 cursor-pointer select-none"
                >
                  {item.name}
                  <span className="pointer-events-none absolute bottom-0 left-0 w-0 h-0.5 bg-[#FF8F5C] transition-all duration-300 group-hover:w-full"></span>
                </a>
              ))}
            </nav>
            ...
            <button
              ...
              className="text-white px-6 py-2 rounded-full font-extrabold transition duration-300 shadow-xl hover:shadow-2xl transform hover:-translate-y-1 text-sm"
          ...
          <nav className="flex flex-col p-4 space-y-2">
            {navItems.map(item => (
              <a
                ...
                href="#"
                onClick={(e) => { e.preventDefault(); handleNavClick(item.page); }}
                className="py-3 px-4 text-gray-800 font-semibold hover:bg-orange-50 rounded-lg transition text-left border-b border-gray-100 cursor-pointer select-none"
              >
                {item.name}
              </a>
            ))}
