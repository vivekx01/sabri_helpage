      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-white">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10 items-start">
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-x-8 gap-y-6 items-stretch">
              {footerLinks.quickLinks.map(link => (
                <button
                  key={link.name}
                  onClick={() => onNavigate(link.page)}
                  className="text-white px-1 py-1.5 transition-all duration-200 text-sm flex items-center text-left leading-tight w-full h-full hover:text-gray-200"
                >
          ...
          <div className="md:col-span-1 md:justify-self-end w-full max-w-sm md:pl-8 lg:pl-16 xl:pl-24">
            <button
        ...
        <div className="border-t border-white/10 pt-6 flex flex-col md:flex-row justify-between items-center md:items-start gap-4">
          ...
          <div className="flex-1 flex md:justify-end justify-center space-x-6 text-xs text-white">
            <button onClick={() => onNavigate('about')} className="transition">About</button>
            <button onClick={() => onNavigate('terms')} className="transition">Terms</button>
            <button onClick={() => onNavigate('privacy')} className="transition">Privacy</button>
          </div>
        </div>
      </div>
    </footer>
