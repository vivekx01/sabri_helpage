        ...
        <div className="flex justify-between items-center py-1 md:py-2">
          ...
          >
            <img 
              ...
              className="object-contain transition-transform hover:scale-105 h-[4rem] md:h-[5rem]"
            />
          </button>
          ...
          <div className="hidden lg:flex items-center space-x-8">
            <nav className="flex space-x-8 text-sm md:text-base font-semibold uppercase tracking-normal text-gray-700">
              {navItems.map(item => (
                ...
                >
                  {item.name}
                  <span className="absolute bottom-0 left-0 w-0 h-0.5 transition-all duration-300 group-hover:w-full" style={{ backgroundColor: COLORS.PRIMARY }}></span>
                </button>
              ))}
            </nav>
          ...
          <nav className="flex flex-col p-4 space-y-2">
            {navItems.map(item => (
              <button
                key={item.name}
                onClick={() => handleNavClick(item.page)}
                className="py-3 px-4 text-gray-800 font-semibold rounded-lg transition text-left border-b border-gray-100"
                style={{ backgroundColor: 'transparent' }}
                onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = COLORS.PRIMARY_PALE; }}
