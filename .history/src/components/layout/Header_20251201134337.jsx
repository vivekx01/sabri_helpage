            ...
            <nav className="flex space-x-8 text-sm md:text-base font-semibold uppercase tracking-normal text-gray-700">
              {navItems.map(item => (
                <button
                  ...
                  className="hover:text-["+COLORS.PRIMARY+"] transition-colors duration-200 relative group py-1"
                >
                  {item.name}
                  <span className="absolute bottom-0 left-0 w-0 h-0.5" style={{backgroundColor: COLORS.PRIMARY}} className="transition-all duration-300 group-hover:w-full"></span>
                </button>
              ))}
            </nav>
          ...
          <nav className="flex flex-col p-4 space-y-2">
            {navItems.map(item => (
              <button
                key={item.name}
                onClick={() => handleNavClick(item.page)}
                className="py-3 px-4 text-gray-800 font-semibold hover:bg-["+COLORS.PRIMARY_PALE+"] rounded-lg transition text-left border-b border-gray-100"
              >
                {item.name}
              </button>
            ))}
