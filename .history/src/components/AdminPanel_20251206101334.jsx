...
const DEFAULT_REGISTRY = [
  ...
  { slug: 'girl-education', name: 'Girl Education' },
  { slug: 'ilc', name: 'ILC' },
  { slug: 'internship', name: 'Internship' },
  { slug: 'our-causes', name: 'Our Causes' },
  { slug: 'privacy', name: 'Privacy' },
  { slug: 'publications', name: 'Publications' },
  { slug: 'sociofare', name: 'Sociofare' },
...
const DEFAULT_PAGE = (slug, name) => {
  // Seed sections that mirror original frontend pages
  switch (slug) {
    case 'home':
      return {
        slug, name,
    ...
    case 'about':
      return {
        ...
        sections: [
          { type: 'paragraph', text: 'Founded in 2013, based in Kolkata. Focus on mental health, elderly care, girl education.' },
          { type: 'paragraph', text: 'Grassroots impact through counselling camps, elderly support, empowerment programs.' },
          { type: 'paragraph', text: 'Dedicated to making society emotionally strong and welcoming.' },
          { type: 'richText', html: '<h3>Governing Body</h3><p>Aarti BR Singh, Yash Guptaa, Prerna Guptaa</p>' },
        ],
        status: 'published',
      };
    case 'sociofare':
      return {
        ...
        meta: { description: 'Community initiatives and outcomes' },
        sections: [
          { type: 'paragraph', text: 'We run initiatives to support communities.' },
          { type: 'richText', html: '<h3>Initiatives</h3><p>Food Drive: Weekly ration support</p>' },
          { type: 'list', items: ['Families Supported: 5,000+'] },
        ],
        status: 'published',
      };
    case 'contact':
      return {
        ...
        meta: { description: 'Get in touch' },
        sections: [
          { type: 'richText', html: '<h3>Offices</h3><p>Kolkata, India</p>' },
          { type: 'richText', html: '<h3>Form</h3><p>Name, Email, Message</p>' },
          { type: 'list', items: ['Twitter', 'YouTube', 'Instagram'] },
        ],
        status: 'published',
      };
    case 'donate':
      return {
        slug, name,
        header: { title: 'Donate' },
        meta: { description: 'Support our work' },
        sections: [
          { type: 'paragraph', text: 'Donation options and transparency' },
          { type: 'list', items: ['₹500', '₹1000', '₹5000'] },
          { type: 'cta', label: 'Donate Now', href: '/donate' },
        ],
        status: 'published',
      };
    case 'our-causes':
      return {
        slug, name,
        header: { title: 'Our Causes' },
        meta: { description: 'Core causes' },
        sections: [
          { type: 'richText', html: '<h2>Causes Grid</h2><ul><li>Mental Health</li><li>Elderly Care</li><li>Girl Education</li></ul>' },
          { type: 'list', items: ['Impact Stats: People helped, Locations'] },
        ], status: 'published',
      };
    case 'mental-health':
    case 'elderly-care':
    case 'girl-education':
      return {
        slug, name,
        header: { title: name },
        meta: { description: `${name} programs and impact` },
        sections: [
          { type: 'paragraph', text: 'Overview of programs' },
          { type: 'richText', html: '<h3>Programs</h3><p>Key initiatives and outcomes</p>' },
          { type: 'list', items: ['Impact stats', 'Partner list'] },
        ], status: 'published',
      };
    case 'awards':
      return { slug, name, header: { title: 'Awards' }, meta: { description: '' }, sections: [{ type: 'richText', html: '<h3>Awards Timeline</h3>' }], status: 'published' };
    case 'blog':
      return { slug, name, header: { title: 'Blog' }, meta: { description: '' }, sections: [{ type: 'richText', html: '<h3>Posts</h3>' }], status: 'published' };
    case 'events':
      return { slug, name, header: { title: 'Events' }, meta: { description: '' }, sections: [{ type: 'richText', html: '<h3>Upcoming & Past Events</h3>' }], status: 'published' };
    case 'faq':
      return { slug, name, header: { title: 'FAQs' }, meta: { description: '' }, sections: [{ type: 'list', items: ['Question? Answer.'] }], status: 'published' };
    case 'gallery':
      return { slug, name, header: { title: 'Gallery' }, meta: { description: '' }, sections: [{ type: 'richText', html: '<h3>Images</h3>' }], status: 'published' };
    case 'ilc':
      return { slug, name, header: { title: 'ILC' }, meta: { description: '' }, sections: [{ type: 'richText', html: '<h3>Services & Resources</h3>' }], status: 'published' };
    case 'internship':
      return { slug, name, header: { title: 'Internship' }, meta: { description: '' }, sections: [{ type: 'richText', html: '<h3>Openings & Benefits</h3>' }], status: 'published' };
    case 'privacy':
      return { slug, name, header: { title: 'Privacy Policy' }, meta: { description: '' }, sections: [{ type: 'richText', html: '<h3>Policy Content</h3>' }], status: 'published' };
    case 'publications':
      return { slug, name, header: { title: 'Publications' }, meta: { description: '' }, sections: [{ type: 'richText', html: '<h3>Papers & Reports</h3>' }], status: 'published' };
    case 'stories':
      return { slug, name, header: { title: 'Stories' }, meta: { description: '' }, sections: [{ type: 'richText', html: '<h3>Stories Grid</h3>' }], status: 'published' };
    case 'terms':
      return { slug, name, header: { title: 'Terms & Conditions' }, meta: { description: '' }, sections: [{ type: 'richText', html: '<h3>Terms Content</h3>' }], status: 'published' };
    case 'csr-summit':
      return { slug, name, header: { title: 'CSR Summit' }, meta: { description: '' }, sections: [{ type: 'richText', html: '<h3>Agenda, Speakers, Registration</h3>' }], status: 'published' };
    default:
      return { slug, name, header: { title: name }, meta: { description: '' }, sections: [{ type: 'richText', html: '' }], status: 'published' };
  }
};
...
const saveRegistry = (list) => {
  try { localStorage.setItem(REGISTRY_KEY, JSON.stringify(list || [])); } catch (_) {}
};
...
const loadPageLocal = (slug, nameFallback) => {
  try {
    ...
    return def;
  } catch (_) { return DEFAULT_PAGE(slug, nameFallback || slug); }
};
