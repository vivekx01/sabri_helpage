...
const ContactPage = ({ onNavigate }) => {
  ...
  const { mutate, loading, error, success, reset } = useApiMutation(api.createContact);
  ...
  return (
    <div className="min-h-screen pb-20" style={{ backgroundColor: COLORS.BG_LIGHT_GRAY }}>
      <PageHeader title="Contact Us" subtitle="We'd love to hear from you" />
      <div className="pt-10 pb-6 text-center">
        <h1 className="text-4xl font-serif font-bold text-gray-900">Contact <span style={{ color: COLORS.PRIMARY }}>Us</span></h1>
      </div>

      <div className="py-12 relative overflow-hidden mb-16" style={{ backgroundColor: COLORS.PRIMARY }}>
        <div className="max-w-5xl mx-auto px-6 flex justify-center items-center relative z-10">
          <h2 className="text-2xl md:text-3xl font-bold text-white">Do you have a Query?</h2>
        </div>
        <div className="absolute right-10 top-1/2 -translate-y-1/2 opacity-30 hidden md:block" style={{ color: COLORS.PRIMARY_PALE }}>
          <div className="text-8xl">✉</div>
        </div>
      </div>

      <div className="max-w-3xl mx-auto px-6 -mt-8 relative z-20 mb-20">
        <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12 border" style={{ borderColor: COLORS.PRIMARY_PALE }}>
            ...
            <div className="text-center pt-2">
              <div className="space-y-3">
                <button
                  ...
                  disabled={loading}
                  className="disabled:opacity-60 text-white px-10 py-4 rounded-lg font-bold text-base transition shadow-lg hover:shadow-xl transform hover:scale-105 duration-200" style={{ backgroundColor: COLORS.PRIMARY }}
                >
                  {loading ? 'Sending…' : 'Send Message ✉'}
                </button>
                {success && <div className="text-sm text-green-600">Message sent — thank you!</div>}
      ...
      <div className="max-w-5xl mx-auto px-6">
        <h3 className="text-2xl font-bold text-center text-gray-900 mb-10">Get in <span style={{ color: COLORS.PRIMARY }}>Touch</span></h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
