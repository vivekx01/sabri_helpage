...
const DonatePage = ({ onNavigate = () => {} }) => {
  ...
  useEffect(() => {
    api.getPageContent('donate').then((data) => {
      setDonate(data);
      setLoading(false);
  ...
  // Use backend data for all texts and options
  const donationOptions = Array.isArray(donate?.donationOptions) && donate.donationOptions.length > 0
    ? donate.donationOptions.map(Number)
    : [5000, 15000, 25000];
            ...
            >
              ...
              <div className="p-6 sm:p-8 lg:p-10 text-center lg:text-left flex flex-col justify-center animate-fadeIn">
                <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-white mb-4 leading-tight">
                  {donate?.heroTitle || 'Transform Lives, Empower Communities'}
                </h1>
                <p className="text-base sm:text-lg text-white/90 max-w-3xl lg:max-w-none mx-auto mb-6 sm:mb-10">
                  {donate?.heroBody || 'Join us in building a compassionate world where every elderly person can thrive with dignity.'}
                </p>
              ...
              <div className="p-4 sm:p-4 lg:p-6 flex flex-col justify-center">
                ...
                >
                  <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6 text-center">{donate?.formTitle || 'Sabri Helpage Donation'}</h2>
                  ...
                  <form className="space-y-4 sm:space-y-5">
                    ...
                    <div>
                      <label htmlFor="support-category" className="block text-sm font-semibold text-gray-700 mb-2">
                        {donate?.supportLabel || 'I offer my support for'}
                      </label>
                      <select
                        id="support-category"
                      ...
                      >
                        <option>{donate?.supportOpt1 || 'Elderly Care'}</option>
                        <option>{donate?.supportOpt2 || 'Education'}</option>
                        <option>{donate?.supportOpt3 || 'Mental Health'}</option>
                        <option>{donate?.supportOpt4 || 'General Fund'}</option>
                      </select>
                    </div>
                    ...
                    <div className="flex justify-between gap-3 sm:gap-4 pt-3">
                      ...
                      <button
                        ...
                        style={isRecurring ? { backgroundColor: COLORS.PRIMARY, borderColor: COLORS.PRIMARY } : {}}
                      >
                        {donate?.recurringLabel || 'Monthly Recurring'}
                      </button>
                    </div>
                    ...
                    <div className="pt-4">
                      ...
                      >
                        {donate?.nextLabel || 'Next'}
                      </button>
                    </div>
                    
                    <p className="text-xs text-gray-500 text-center pt-2">
                      {donate?.taxNote || 'All Donations to Sabri Helpage are tax exempted.'}
                    </p>
                  </form>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="bg-white p-4 rounded-xl shadow-md border-l-4 transition-all duration-300 hover:shadow-lg" style={{ borderColor: COLORS.PRIMARY_DARK }}>
                    ...
                    <p className="text-xs text-gray-500">
                      {donate?.summaryBody || 'We ensure that your donation reaches those who need it most, with complete transparency and accountability.'}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="max-w-4xl mx-auto bg-white p-4 sm:p-6 lg:p-10 rounded-2xl shadow-2xl border-t-8" style={{ borderColor: COLORS.PRIMARY_DARK }}>
            ...
            <div className="bg-gray-50 p-4 rounded-xl mb-6 sm:mb-8 border border-gray-200 transition-all duration-300 hover:shadow-md">
              <h3 className="text-base sm:text-lg font-bold text-gray-700 flex items-center mb-2">
                <Wallet className="w-5 h-5 mr-2 text-green-600" /> {donate?.summaryTitle || 'Donation Summary'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex flex-col sm:flex-row gap-4 pt-4">
                ...
                >
                  {donate?.backLabel || 'Back to Amount'}
                </button>
                <button
                  type="submit"
                  disabled={
                ...
                >
                  {(donate?.payLabel || 'Pay')} ₹{finalAmount.toLocaleString('en-IN')} {(donate?.paySuffix || 'Now')}
                </button>
              </div>
            </form>
          </div>
