...
import { getPage } from '../services/api.mjs';
...
const HomePage = ({ onNavigate }) => {
  ...
  const heroImage = page.heroImage || IMAGE_URLS.HERO;
  const heroTitle = page.heroTitle || 'Serving society for more than a decade.';
  const heroContent = page.heroContent || '';
  const supporters = ["Google", "Microsoft", "Tata Trusts", "Reliance Foundation", "HDFC Bank"];
  // ...existing code for stories/newsItems can be adapted from page.sections if needed
    ...
    <>
      <section className="relative h-[600px] md:h-[700px] flex items-center justify-center text-white text-center overflow-hidden bg-black">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 animate-fade-in-up">
          ...
          {heroContent && (
            <p className="mb-6 text-lg md:text-2xl font-medium text-white/90">{heroContent}</p>
          )}
