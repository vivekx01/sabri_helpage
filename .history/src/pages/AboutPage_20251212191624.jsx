...
const AboutPage = ({ onNavigate }) => {
  ...
  return (
    <PageLayout page="about" onNavigate={onNavigate}>
      <div className="space-y-10 text-gray-800">
        <div className="bg-white rounded-2xl shadow-md p-8 md:p-10">
          <h2 className="text-3xl font-bold mb-6" style={{ color: '#b4362c' }}>Our Story</h2>
          <div className="space-y-5 text-lg leading-relaxed">
            {about.sections.map((section, index) => (
              <p key={index} className="text-gray-700">{section.text}</p>
            ))}
          </div>
        </div>

        <div className="bg-white rounded-2xl shadow-md p-8 md:p-10">
          <h2 className="text-3xl font-bold mb-6" style={{ color: '#b4362c' }}>Governing Body</h2>
          <div className="space-y-10">
            {about.governingBody.map((member, index) => (
              <div key={index} className="border-b border-gray-200 pb-8 last:border-0 last:pb-0">
                <h3 className="text-2xl font-bold mb-4" style={{ color: '#b4362c' }}>{member.name}</h3>
