...
const ReadMorePage = ({ onBack }) => (
 ...
 const ElderlyIcon = (props) => (
    <svg className="w-10 h-10 mx-auto text-green-600 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" {...props}>
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3v-1m18 0h-1a2 2 0 00-2 2v4a2 2 0 01-2 2H6a2 2 0 01-2-2v-4a2 2 0 00-2-2H3" />
    </svg>
);
const EducationIcon = (props) => (
    <svg className="w-10 h-10 mx-auto text-blue-600 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" {...props}>
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" />
    </svg>
);
const HealthIcon = (props) => (
    <svg className="w-10 h-10 mx-auto text-red-600 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" {...props}>
...
const VerifiedIcon = (props) => (
    <svg className="w-4 h-4 mr-1 text-green-600" fill="currentColor" viewBox="0 0 20 20" {...props}>
...
// --- Your Original Donation Form Logic (Renamed and simplified for inclusion) ---
// Note: This component assumes 'onDonate' handles moving to the next step,
// which is a good pattern for the smaller form in the page sidebar.
const HomeDonationForm = ({ onDonate }) => {
    const [amount, setAmount] = useState(5000);
    const [customAmount, setCustomAmount] = useState('');
    const [supportCategory, setSupportCategory] = useState('Elderly Care');
    
    const donationOptions = [5000, 15000, 25000]; // Updated to match image options
    ...
    const handleNext = () => {
        ...
        if (amt <= 500) return; // Check against the minimum amount mentioned in the image
        
        // Call the parent handler to switch the page/modal to the next step
        onDonate({
            amount: amount,
    ...
    const isAmountSelected = (val) => amount === val && !customAmount;
        ...
        <div className="p-6 rounded-xl shadow-2xl" style={{ backgroundColor: 'white', border: '1px solid #e5e7eb' }}>
                ...
                {donationOptions.map((opt) => (
                    <button
                        ...
                        className={`flex-1 px-4 py-2 rounded-lg text-base font-bold transition duration-200 border-2 ${
                            isAmountSelected(opt)
                                ? 'text-white shadow-md'
                                : 'text-gray-700 border-gray-300 hover:border-gray-500'
                        }`}
                        style={isAmountSelected(opt) ? { backgroundColor: ACCENT_ORANGE } : { backgroundColor: 'white' }}
                    >
            ...
            <div className="relative mb-4">
                <input
                    ...
                    className={`w-full p-3 border-2 rounded-lg focus:outline-none focus:ring-2 ${isCustomActive ? 'border-gray-500' : 'border-gray-300'}`}
            ...
            {/* Regular Users Login (Placeholder) */}
            ...
            <button
                ...
                style={{ backgroundColor: ACCENT_ORANGE }}
            >
                Next {finalAmount > 0 && `(₹${finalAmount.toLocaleString('en-IN')})`}
            </button>
...
// --- Full DonatePage Component (Rendering the whole page) ---
const DonatePage = ({ onBack, initialData }) => {
    // State to manage showing the details form (Step 2) as a separate view/modal
    // For this full-page layout, we'll keep it simple and just show the main donation form
    // The actual state management from your original code would typically go here or in a parent component
    
    // We'll use a local state to simulate the "Next" button in HomeDonationForm initiating the full process
    const [showDetailsForm, setShowDetailsForm] = useState(false);
    const [donationInfo, setDonationInfo] = useState(initialData || null);

    const handleInitiateDonation = (data) => {
        setDonationInfo(data);
        setShowDetailsForm(true); // Switch to the details view (Step 2)
        // In a real app, you might use a modal or route change here.
    };

    // If the user clicks 'Next' in the sidebar form, or if initialData is present, show the full details form.
    if (showDetailsForm) {
        // Here we render the full details form (Step 2 of your original logic)
        // Note: I'm skipping the complex state management from your original code block 
        // to simplify this example, but you would substitute this with your full Step 2 component.
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-100 p-8">
                {/* This is a placeholder for your full Step 2 component */}
                <div className="bg-white p-10 rounded-xl shadow-2xl max-w-2xl w-full">
                    <h2 className="text-3xl font-extrabold text-gray-900 mb-4">Complete Your Donation</h2>
                    <p className="text-lg text-gray-600 mb-6">You're donating **₹{donationInfo?.customAmount || donationInfo?.amount}** for **{donationInfo?.category}**.</p>
                    {/* Placeholder for the full Step 2 form (First Name, PAN, Address, etc.) */}
                    <div className="h-64 border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-500">
                        [Donor Details Form (Your original Step 2 code) goes here]
                    </div>
                    <button onClick={() => setShowDetailsForm(false)} className="mt-6 text-sm text-gray-600 hover:text-gray-900">
    ...
    return (
        <div className="donate-page-full-layout min-h-screen">
            ...
            <header className="py-12 md:py-16 text-center text-white" style={{ backgroundColor: PRIMARY_ORANGE }}>
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <main className="py-16 md:py-20" style={{ backgroundColor: '#F8F8F8' }}> {/* Light gray background for contrast */}
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                        ...
                        {/* Right Column: Donation Form (Your original Step 1) */}
                        <div className="lg:col-span-1">
                            {/* RENDER THE HOME DONATION FORM COMPONENT */}
                            <HomeDonationForm onDonate={handleInitiateDonation} />
                        </div>
                    </div>
                </div>
            </main>
            ...
            <footer className="py-6 bg-gray-800 text-white text-center">
                <p className="text-sm">© 2025 Sabri Helpage. All rights reserved.</p>
            </footer>
        </div>
    );
};
