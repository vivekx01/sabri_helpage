...
const ReadMorePage = ({ onBack }) => (
 ...
 const DonatePage = ({ onBack, initialData }) => {
    ...
    const [isRecurring, setIsRecurring] = useState(false); // Note: Frequency is not in the sidebar form image
    const [supportCategory, setSupportCategory] = useState(initialData?.category || 'Elderly Care');
    
    // --- Step 2 State (from your existing code) ---
    const [step, setStep] = useState(initialData ? 2 : 1); // Controls the view: 1 = full page with sidebar form; 2 = full Step 2 form
    ...
    // Updated options to match the image: 5,000, 15,000, 25,000
    ...
    const handleCustomAmountChange = (e) => {
        ...
        setAmount(0); // Set fixed amount to 0 when custom is active
    };

    // Use customAmount if present, otherwise use amount. Fallback to 0.
    ...
    // Check if custom input is active (amount > 0 and not a fixed option)
    const isCustomActive = finalAmount > 0 && !isFixedAmountSelected;

    // --- Step 1 -> Step 2 Navigation ---
    const handleNext = () => {
        if (finalAmount < 500) { // Using the minimum amount from the image
            alert("Minimum donation amount is ₹500/-");
            return; 
        }
        // Save initial data for Step 2 if needed (optional)
        // setDonationInfo({ finalAmount, supportCategory, isRecurring });
        setStep(2); // Go to the Step 2 full-screen form
    };
    ...
    const handleSubmit = (e) => {
        ...
        // This is where you'd mock the successful payment and navigate back
        alert("Donation successful! Check console for data.");
        onBack(); 
    };
    
    // --- RENDER STEP 2: Full-screen Donor Details Form (Your original large component) ---
    if (step === 2) {
        // This section renders your original full Step 2 logic, adapted to be a standalone view.
        return (
                    ...
                    <div className="bg-white p-6 sm:p-10 rounded-2xl shadow-2xl border-t-8" style={{borderColor: PRIMARY_DARK}}>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8">
                            <div className="space-y-6">
                                ...
                                {/* Citizen Type (Copied from your original Step 2 code) */}
                                <div>
                                ...
                                {/* Name Grid (Copied from your original Step 2 code) */}
                                ...
                                {/* Contact Grid (Copied from your original Step 2 code) */}
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                ...
                                {/* DOB / PAN Grid (Copied from your original Step 2 code) */}
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                ...
                                {/* Address Fields (Copied from your original Step 2 code) */}
                                <div>
                                ...
                                {/* Existing Donor (Copied from your original Step 2 code) */}
                                <div>
            ...
            <main className="py-16 md:py-20" style={{ backgroundColor: BG_LIGHT_GRAY }}>
                <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                        ...
                        {/* Right Column: Donation Form (Your code adapted for the sidebar) */}
